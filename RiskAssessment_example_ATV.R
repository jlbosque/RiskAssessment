# #######################################################################################################################
# This script takes you through an analysis of the ATV example that appears in "Quantitative system risk assessment
...
# #######################################################################################################################
# The tree for the ATV example is specified in the file tree_ATV.conf and a tree object is created.
tree <- create_fault_tree("tree_ATV.conf")
# #######################################################################################################################
...
# Three separate eliciations are done for the ATV.  Here we do that by 3 separate calls to this
# function which we then merge
beta_parameters_1 <- derive_primary_event_priors(cornerstone_event_index    = 1,
                                                 cornerstone_event_interval = c(0.01,0.04),
                                                 pairwise_matrix = matrix(c(1.00, 1.04, 1.00,
                                                                            0.53, 1.00, 0.53,
                                                                            1.00, 1.04, 1.00), nrow=3,byrow=TRUE))
beta_parameters_2 <- derive_primary_event_priors(cornerstone_event_index    = 1,
                                                 cornerstone_event_interval = c(0.005,0.02),
                                                 pairwise_matrix = matrix(c(1.00, 1.23,
                                                                            0.28, 1.00), nrow=2,byrow=TRUE))
beta_parameters_3 <- derive_primary_event_priors(cornerstone_event_index    = 1,
                                                 cornerstone_event_interval = c(0.014,0.055),
                                                 pairwise_matrix = matrix(rep(1.00,36), nrow=6,byrow=TRUE))
beta_parameters <- rbind(beta_parameters_1,beta_parameters_2,beta_parameters_3)
...
#   lty: line type, following usual R plotting parameter lty
...
data <- simulate_data(n_simulated_data = 5, 
                      ...
                      data_type        = "top_only",
                      true_primary_p   = c(0.015,0.0072,0.015,0.0069,0.0015,0.023,0.023,0.023,0.023,0.023,0.023), 
                      p_obs            = 0.5)
#   where:
...
data <- as.matrix(read.csv(file="sample_data_ATV.csv",sep=","))
colnames(data) <- NULL
# Thirdly, you can just directly enter the matrix of data
data <- matrix(rep(c(rep(NA,tree$n_nodes-1),0),5), 
...
posterior <- inference(beta_parameters, data, data_type="top_only", MCMC_parameters, tree)
# ########################################################################################################################
